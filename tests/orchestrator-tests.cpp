...
TEST(UDPOrchestratorTests, LightMultishardWASM)
{
    std::vector<uint8_t> packet_1 = read_binary_file("tests/packets/test-packet-1.bin");
    size_t packet_size = packet_1.size();
    ...
    Orchestrator<UDPSession>::MessageHandlerFactory handler_factory =
        [engine, module]() -> std::unique_ptr<MessageHandler>
            {
                return std::make_unique<WASMMessageHandler>(engine, module);
            };
    ...
    // Configure the orchestrator, set to use 2 shards.
    OrchestratorConfig<UDPSession> orchestrator_config({4, 12288, true, false, 100},
                                                       host_info,
                                                       handler_factory,
                                                       2);
    ...
    // Create one SEND per payload to prevent packet loss.
    for (int i = 0; i < BASE_NUM_PAYLOADS; i++)
    {
    ...
    Orchestrator<UDPSession> orchestrator(actions,
                                          payloads,
                                          steps,
                                          orchestrator_config,
                                          metric_sink);
